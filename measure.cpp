...
int measure(struct pt *pt, struct measure_stack &m){
  ...
  //Log to the SD card
  m.maxtemp = std::max(m.latest.upper, m.maxtemp); MARK;
  m.maxtemp = std::max(m.latest.lower, m.maxtemp); MARK;
  m.maxtemp = std::max(m.latest.heater, m.maxtemp); MARK;
...
int delta(struct pt *pt, struct measure_stack &m, char &rdv)
{
  ...
  if(m.treeID){
    inrange(m.treeID, "Heater Peak Temperature", m.latest.heater,
            m.reference.heater+0.5, m.reference.heater+5);
