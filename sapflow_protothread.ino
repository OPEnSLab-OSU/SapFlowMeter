...
const int thd_count = 1; //number of sensors installed.
const float batteryThreshold = 1000; //battery threshold value, will cut power if exceeded value in mAh;
...
This function is called when the microcontroller first starts up or is reset.
It's good for things that should only happen once. You do not need to call it
yourself. It initializes some hardware, puts the protothreads in a known state,
and begins the measurement cycle (which starts with sleep)
*/
void setup() {
...
2. Measure temperature of tree (before heat is applied)
3. Apply heat pulse
4. Wait for the peak of the heat to reach the upper and lower probes.
5. Measure the sap flow
6. Sleep until next measurement cycle
...
int schedule(struct pt * pt){
  PT_BEGIN(pt);
  //First run check if battery level is above a certain parameter
  if(batteryRead(batteryThreshold)){
    digitalWrite(KILL_SWITCH, LOW);
  }
  static char rendezvous;
  // Avoid transmitting at the same time as another datalogger
  int offset = 5 * m1.treeID;
